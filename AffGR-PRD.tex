...
Even though a comparison between our affine model and GR does not require the imposition of vanishing torsion, for the sake of simplicity, at this stage we shall focus on a sector of the theory in which the connection is compatible with a metric, and torsion-free. Notice that the vanishing torsion limit ---equivalent to taking $T^{\lambda,\mu\nu} \to 0$ and $A_\mu \to 0$--- cannot be taken at the action level, and should be taken in the field equations. Interestingly, the field equations from Eq.~\eqref{4dfull} can be consistently \emph{truncated} under such requirements, because only the terms of the action linear in these fields will be relevant, i.e.,
\begin{equation*}
  %\label{eff-action}
