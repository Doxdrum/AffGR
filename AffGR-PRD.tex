...
From the geodesic equations and the linearized action on scalar perturbation, we found that only a few terms in the scalar perturbation affect the geodesic equation. The effective geodesic deviation is given by
\begin{equation}
  \label{NewtonPot}
...
Even though a comparison between our affine model and GR does not require the imposition of vanishing torsion, for the sake of simplicity, at this stage we shall focus on a sector of the theory in which the connection is compatible with a metric, and torsion-free. Notice that the vanishing torsion limit ---equivalent to take $T^{\lambda,\mu\nu} \to 0$ and $A_\mu \to 0$--- cannot be taken at the action level, and should be taken in the field equations. Interestingly, the field equations from Eq.~\eqref{4dfull} can be consistently \emph{truncated} under such requirements, because only the terms of the action linear in these fields will be relevant, i.e.,
\begin{equation*}
  %\label{eff-action}
  S_{\text{eff}} = \int\dn{4}{x} \Big( C_1\, R_{\mu\rho}{}^{\mu}{}_\nu \nabla_\sigma T^{\nu,\rho\sigma} + C_2 \, R_{\mu\nu}{}^{\rho}{}_\rho \nabla_\sigma T^{\sigma,\mu\nu} \Big),
\end{equation*}
and the only nontrivial field equation is the one for the Curtright, $T^{\nu,\mu\rho}$,
\begin{equation*}
...
For a Riemannian manifold $(\Mi,g)$ the Eq.~\eqref{SimpleEOM} is related through the first Bianchi identity to the harmonic curvature condition~\cite{bourguignon1981varietes},
\begin{equation}
  \label{harm-curv}
  \nabla_\lambda R_{\mu\nu}{}^\lambda{}_\rho = 0,
\end{equation}
and it can be shown that a manifold with harmonic curvature is equivalent (in four dimensions) to a manifold with harmonic Weyl tensor~\cite{Berger:1969} and constant scalar curvature, or in other words the Ricci tensor is a Codazzi tensor~\footnote{A Codazzi tensor is a symmetric $(0,2)$-type tensor, $T$, satisfying the condition \mbox{$D_X T(Y,Z) = D_Y T(X,Z)$~\cite{Derdzinski01071983}.}}. For prove of these equivalences, see Refs.~\cite{Derdzinski:1985,Besse}.
