...
%%\hl{Are we sure of this??? It does not sound true.}

%%%%%%%%% MATTER %%%%%%%%%
Until now, the most general diffeomorphism-invariant and power-counting renormalizable (gravitational) theory for an affine connection has been built, and we have showed that in a certain sector it is equivalent to GR. In what follows, we show an attempt of including scalar matter into the model. The theory does not require a spacetime metric, but instead we consider the most general $\binom{2}{0}$-tensor density, $\mathfrak{g}^{\mu\nu}$,  built with the available fields, an use it to build Lagrangian densities for the matter content. Following our precept, the matter content --- scalar matter --- should couple to $\mathfrak{g}^{\mu\nu}$, given by %% which for Eq.~\eqref{4dfull} is
\begin{dmath}
...
Moreover, our gravitational model coupled to a scalar field through the \emph{density tensor} $\mathfrak{g}^{\mu\nu}$ --- in the vanishing torsion consideration ---, generalises GR minimally coupled to a massless scalar field. In a forthcoming paper, we will consider more general matter content, and some properties of the field equations which differ from the ones of GR.%%\hl{JZ: Ok, so?}
